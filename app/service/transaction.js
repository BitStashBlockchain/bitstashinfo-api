...
class TransactionService extends Service {
  async getTransaction(id) {
    ...
    const {Address: RawAddress} = this.app.bitstashinfo.lib
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.bitstashinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.bitstashinfo.rpc(this.app.config.bitstashinfo.rpc)
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.bitstashinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.bitstashinfo.lib
  ...
  async transformQRC20Transfers(outputs) {
    const TransferABI = this.app.bitstashinfo.lib.Solidity.qrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
  ...
  async transformQRC20UnconfirmedTransfers(outputs) {
    const {OutputScript, Solidity} = this.app.bitstashinfo.lib
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.bitstashinfo.lib
    const {
